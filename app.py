...
<style>
    ...
    .stButton > button {
        ...
        font-size: 32px !important; /* 글자 크기 조절 */
...
# Modal / Fullscreen view for 🏆 전체 당첨자
if st.session_state.show_all_modal:
    st.markdown("---")
    st.markdown("## 🏆 전체 당첨자 그룹별 보기")
    ...
    if not st.session_state.winners:
        ...
        with col_nav1:
            if st.button("◀ 이전 그룹", disabled=(c_page <= 0)):
                st.session_state.all_winners_page -= 1
                st.rerun()
        with col_nav2:
            st.markdown(f"<h3 style='text-align: center; color: #ffd700;'>추첨 그룹 {c_page + 1} / {len(group_keys)} ({current_winners[0].get('prize', '경품')})</h3>", unsafe_allow_html=True)
        with col_nav3:
            if st.button("다음 그룹 ▶", disabled=(c_page >= len(group_keys) - 1)):
                st.session_state.all_winners_page += 1
                st.rerun()
        with col_nav4:
        ...
        # Grid display of current group winners
        cols_per_row = 4
        ...
        for row in rows_data:
            ...
            for idx, w in enumerate(row):
                with cols[idx]:
                    st.markdown(f"""
                    <div style="background-color: #1e293b; border: 2px solid #334155; border-radius: 12px; padding: 20px; text-align: center; margin-bottom: 15px;">
                        <p style="color: #94a3b8; font-size: 0.9rem; margin-bottom: 5px;">{w.get('draw_number', 1)}번</p>
                        <h2 style="color: white; font-size: 2rem; margin: 5px 0;">{w['name']}</h2>
                        <p style="color: #cbd5e1; font-size: 1.2rem; margin-top: 5px;">{w['id']}</p>
                    </div>
                    """, unsafe_allow_html=True)
    st.markdown("---")
